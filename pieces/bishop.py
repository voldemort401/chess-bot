def bishop(self,piece_pos: str, board: list[str], q=0 ) -> list[str] | int: 
        if (board[self.board_sqs.index(piece_pos)] != self.BBISHOP and board[self.board_sqs.index(piece_pos)] != self.WBISHOP and q == 0):
            return -1 
        current_pos = self.board_sqs.index(piece_pos)
        color       = board[current_pos][:3]
        PIECE_POS = self.board_sqs.index(piece_pos)
        possible_pos = [] 
        pve9 = True 
        pve7 = False 
        ...
        for i in range(8):
            if (self.board_sqs[current_pos] in self.edges):
            ...
            if (pve9):    
                current_pos+=9
                if (board[current_pos] == self.EMPTY):
            ...
            if (pve7):
                ...
                if (board[current_pos] == self.EMPTY):
                    ...
                    pve7 = False
                elif (self.board_sqs[current_pos] in self.edges):
                    possible_pos.append(current_pos)
                    pve7 = False
